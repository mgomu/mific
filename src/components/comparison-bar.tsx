...
export function ComparisonBar({ selectedFunds, onRemove }: ComparisonBarProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full flex justify-center items-center pb-8 px-6 z-[60]">
      <div className="bg-white/90 backdrop-blur-lg shadow-[0_-8px_30px_rgba(20,27,43,0.08)] rounded-2xl py-4 px-8 flex items-center justify-between gap-12 max-w-[800px] w-full border border-white/20">
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2">
            {selectedFunds.map((fund) => (
              <div
                key={fund.id}
                className="bg-primary-fixed/50 px-3 py-1 rounded-full flex items-center gap-2 text-xs font-semibold text-on-primary-fixed-variant"
              >
                {fund.name}
                <button
        ...
        <Link
          ...
          className={`rounded-full px-8 py-3 font-semibold text-sm flex items-center gap-2 transition-opacity ${
            canCompare
              ? "bg-gradient-to-r from-[#004ac6] to-[#2563eb] text-white hover:opacity-90 active:scale-95"
